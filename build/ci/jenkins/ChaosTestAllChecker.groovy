pipeline {
    ...
    stages {
                ...
                container('main') {
                    dir ('tests/python_client/chaos') {
                        ...
                        if ("${params.chaos_task}" == "chaos-test"){
                            def host = sh(returnStdout: true, script: "kubectl get svc/${env.RELEASE_NAME}-milvus -o jsonpath=\"{.spec.clusterIP}\"").trim()
                            sh "pytest -s -v test_chaos_all_checkers.py --host $host --log-cli-level=INFO --capture=no || echo 'chaos test fail' "
                        }
        ...
        stage ('Verify all operations for checker collections after chaos') {
            options {
              timeout(time: 10, unit: 'MINUTES')   // timeout on this stage
            }
            steps {
                container('main') {
                    dir ('tests/python_client/chaos') {
                        ...
                        sh "pytest testcases/test_all_operations.py --host $host"
                        sh "kubectl get pods|grep ${env.RELEASE_NAME}"
                        }
                    }
                }
