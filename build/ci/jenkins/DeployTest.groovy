                ...
                container('main') {
                    dir ('tests/python_client/deploy') {
                        script {
                            ...
                            sh "helm repo add milvus https://charts-dev.zilliz.cc"
                            sh "helm repo update"
                            if ("${params.milvus_mode}" == "standalone") {
                                sh "helm install --wait --timeout 720s ${env.RELEASE_NAME} milvus/milvus  --set image.all.repository=${params.old_image_repository} --set image.all.tag=${old_image_tag_modified} -f standalone-values.yaml;"    
                            }
                            if ("${params.milvus_mode}" == "cluster") {
                                sh "helm install --wait --timeout 720s ${env.RELEASE_NAME} milvus/milvus  --set image.all.repository=${params.old_image_repository} --set image.all.tag=${old_image_tag_modified} -f cluster-values.yaml;"    
                            }                            
